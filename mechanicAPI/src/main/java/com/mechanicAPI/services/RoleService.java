...
public static SessionFactory factory = HibernateSession.factory;
	...
	public Roles updateRole(Integer roleID, Roles role) {
	    ...
	    Roles object = new Roles();
	      ...
	      try {
	         tx = session.beginTransaction();
	         object = (Roles) session.get(Roles.class, roleID); 
	...
	public Roles deleteRole(Integer roleID, Integer newRoleID) {
		
		int oldRoleAmmount = 0;
		Session session = factory.openSession();
	    Transaction tx = null;
	    Roles deleteObject = new Roles();
	      try {
	         tx = session.beginTransaction();
	         ...
	         deleteObject = (Roles) session.get(Roles.class, roleID); 
